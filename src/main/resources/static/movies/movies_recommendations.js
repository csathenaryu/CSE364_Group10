...
function get_query(){
    ...
    // let url = "http://localhost:8080/movies/recommendations.html?title=Sanjuto+(1962)&limit=20";
...
const movieUrlHead = "http://localhost:8080/movies/recommendations";
const parameters = get_query();

const urlencoder = new UrlEncoder();
const movieUrl = urlencoder.getUrl(movieUrlHead, parameters);

async function loading(movieUrl) {
    const movieContainer = new MovieContainer();
    console.log(movieUrl);
    const list = await getRecommendedMovies(movieUrl);
    console.log(list);
